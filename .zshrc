...
alias weather="curl wttr.in -H 'accept-language: zh-CN'"
...
#   sudo route add -net 0.0.0.0 192.168.20.1
#   sudo route add 192.168.0.0/16 192.168.30.2
# }

# route() {
#   local _command="$1"
#   if [[ "$_command" == "proxy" ]]; then
#     _proxy_route
#   else
#     command route $@
#   fi
...
brew() {
  local _opt="$1"
  if [[ "$_opt" == "cask" ]]; then
    # prepend "--cask" flag between "brew" and commands
    local _op="$2"; shift 2
    command brew $_op --cask $@
  elif [[ "$_opt" == "update" ]] || [[ "$_opt" == "search" ]]; then
    # always apply http(s)_proxy when "update" or "search"
    local _op="$1"; shift 1
    proxy; command brew $_op $@; unproxy
  else
